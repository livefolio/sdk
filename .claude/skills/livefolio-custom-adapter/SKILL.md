...
Real-world example: `AlpacaDataFeed` (in `~/Documents/Personal/livefolio-2/alpaca/src/alpaca-data-feed.ts`) implements this interface over Alpaca's `/v2/stocks/{symbol}/bars` REST endpoint. It illustrates range-aware in-memory caching, in-flight dedup, total-return adjustment, and SDK→Alpaca timeframe translation.
...
Real-world example: `AlpacaStreamingDataFeed` (in `~/Documents/Personal/livefolio-2/alpaca/src/alpaca-streaming-data-feed.ts`) implements this interface over Alpaca's WebSocket trades stream. It wraps a ref-counted `AlpacaStreamClient` and converts each `AlpacaTradeMessage` into a `StreamingBar` (degenerate bar at trade price), illustrating reconnect-with-backoff, multi-asset subscription management, and the "raw ticks only — no aggregation" rule.
...
Real-world example: `AlpacaExecutor` (in `~/Documents/Personal/livefolio-2/alpaca/src/alpaca-executor.ts`) implements this interface over Alpaca's `/v2/orders` REST endpoint, routing each SDK `Order` variant to a market order and polling for terminal status before returning `Fill[]`. It illustrates `Order` discriminated-union handling, idempotency via `client_order_id`, and configurable poll timeout.
